...
import { normalizeArea, normalizeLayoutRelations } from "./normalizeLayoutRelations.js";

const SUPPORTED_VIEWPORT_MODES = new Set(["narrow", "mobile"]);

export function applyLayoutRelationManualAreaCommand({
  items = [],
  parentId,
  childId,
  viewportMode,
  ...
  if (!SUPPORTED_VIEWPORT_MODES.has(resolvedViewportMode)) {
    return createErrorResult({
      sourceItems,
      message: "Manual area supports only narrow or mobile viewport modes."
    });
  }
  ...
  if (!normalizedArea) {
    return createErrorResult({
      sourceItems,
      message: "Manual area is invalid."
    });
  }
  ...
  return createAdapterResult({
    ...
    message: `Manual ${resolvedViewportMode} area saved for relation child "${resolvedChildId}".`,
    data: {
      items: nextItems,
      changed: true,
...
function patchParentManualArea({ item, childIndex, viewportMode, area }) {
