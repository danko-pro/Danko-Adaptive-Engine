...
import { resolveSidebarMobileButtonRelativeArea } from "./resolveSidebarMobileButtonRelativeArea.js";

export const SIDEBAR_MOBILE_BUTTON_POINTER_TYPES = {
  MOVE: "move",
  RESIZE: "resize"
};

export function createSidebarMobileButtonPointerInteraction({
  event,
  ...
  const buttonArea = normalizeArea(renderInfo?.mobilePresentation?.buttonArea);
  const renderArea = normalizeArea(renderInfo?.renderArea);

  if (!buttonArea || !renderArea) {
    return null;
  }
...
export function createSidebarMobileButtonPointerMove({
  event,
  interaction,
  metrics
} = {}) {
  ...
  const absoluteArea = resolveAbsoluteAreaFromOperation({
    operation,
    startArea: interaction?.startButtonArea
  });
  const relativeArea = resolveSidebarMobileButtonRelativeArea({
    absoluteArea,
    renderArea: interaction?.renderArea
  });
