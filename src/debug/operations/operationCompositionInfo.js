...
function formatCompositionDetails(info) {
  return [
    `С‚РёРї: ${info.type}`,
    `РїРѕР·РёС†РёСЏ: ${info.position}`,
    info.groupId ? `РіСЂСѓРїРїР°: ${info.groupId}` : "",
    info.anchors ? `РїСЂРёРІСЏР·РєР°: ${info.anchors}` : "",
    info.relation ? `СЃРІСЏР·СЊ: ${info.relation}` : "",
...
function formatRecommendation(plan, blockId) {
  ...
  return "РЎРѕС…СЂР°РЅРёС‚СЊ СЂРѕР»СЊ Рё РјРµСЃС‚Рѕ РІ РєРѕРјРїРѕР·РёС†РёРё.";
}
